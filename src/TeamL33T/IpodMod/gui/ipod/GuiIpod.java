...
public class GuiIpod extends GuiScreen {
	...
	public void actionPerformed(GuiButton button) {
		...
		case 0:
			...
			if (Main.Player.getPlayerState() == 0 && Main.Player.secondCall) {
				print("Stopped (not first-time) is resumed");
				Main.Player.resumeSound();
				updateBtnList(false);
			} else if (Main.Player.getPlayerState() == 1) {
				...
				setWavFile(0);
				playIpod();
				Main.Player.secondCall = true;
				updateBtnList(false);
			}
			break;
		case 1:
			print("Pausing...");
			Main.Player.pauseSound();
			updateBtnList(true);
			break;
		case 2:
			print("Stopping...");
	...
	private void setWavFile(boolean isNext) {
		...
		Main.Player.setFilename(soundList[currentSongIndex], currentSongIndex);
	...
	private void setWavFile(int index) {
		...
		Main.Player.setFilename(soundList[index], index);
	...
	private void updateBtnList() {
		int pState = Main.Player.getPlayerState();
